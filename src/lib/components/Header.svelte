...
<div class="relative container mx-auto">
  ...
  <header class="flex items-center pt-2">
    ...
    <div class="z-10 flex-1 hidden sm:flex space-x-4 lg:space-x-8 w-full justify-end ">
      ...
      <a class="text-base-content text-opacity-80 hove:text-opacity-100 menu-link font-medium font-title text-sm md:text-md lg:text-lg hover:text-primary transition" to={item.path}>
        { item.title }
      </a>
      {/each}
    </div>
