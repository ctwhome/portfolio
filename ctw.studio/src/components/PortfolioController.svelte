  ...
  onMount(() => {
    ...
    function openProject(id: string, navigate = false, trigger?: HTMLElement | null) {
      ...
      if (navigate && idFromHash() !== id) {
        if (switching) {
          const state = history.state?.[galleryStateKey]
            ? { ...history.state, project: id }
            : history.state;
          ...
          history.pushState({ ...baseState, [galleryStateKey]: true, project: id }, '', `#${id}`);
        }
      }
    }
    ...
    return () => {
      ...
      dialogs.forEach((dialog) => dialog.removeEventListener('close', onDialogClose));
      window.removeEventListener('popstate', syncFromLocation);
      window.removeEventListener('hashchange', syncFromLocation);
      closeActive(false);
      html.classList.remove('portfolio-enhanced');
