...
export default {
  ...
  async asyncData ({ $content, params }) {
    const post = await $content('product-design', params.post).fetch()
    const [prev, next] = await $content('product-design')
      .only(['title', 'post'])
      .sortBy('created', 'desc')
  ...
  methods: {
    goToTag (tag) {
      this.$router.push({ path: '/product-design', query: { tag, status: this.$route.query.status } })
    },
    formatDate (date) {
